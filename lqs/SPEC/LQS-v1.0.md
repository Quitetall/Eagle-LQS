# LQS v1.0 — A Universal Standard for EEG Signal Compression
...
This document is the **frozen, versioned normative specification** for LQS v1.0.
...
LQS grades a codec through its **public encode/decode boundary only**. It never
...
- **Blob.** The opaque byte string a codec's `encode` produces and its `decode`
  consumes. LQS imposes no structure on the blob.
...
- **Grader.** A conforming implementation of this spec that drives a codec and
  produces an `LqsReport`. The Rust crate beside this file is the **reference
...
variables `LQS_CHANNELS`, `LQS_SAMPLES`, `LQS_RATE`, `LQS_DTYPE` (a codec MAY read
either). The grader also sets `LQS_WORKDIR` to the per-invocation scratch dir.
...
cmd = "python3"              # binary/script; resolved path → $LQS_CODEC_<NAME>_BIN → PATH
...
[codec.env]                  # extra env merged over the LQS_* variables
PYTHONUNBUFFERED = "1"
```
...
with `eagle-lqs emit-corpus-manifest` and grades against the resulting manifest.
...
> **Normative.** Schemas in `SPEC/schemas/lqs-report-v1.json` and
> `SPEC/schemas/lqs-submission-v1.json`.
...
`LqsReport` per file, the corpus summary, the leaderboard ordering, and an
**optional** `task_concordance` block (§10). Every `LqsReport` carries a
`spec_version` field so it is self-identifying out of context.

A conforming submission MUST validate against `lqs-submission-v1.json`. A grader
...
LQS v1.0 defines an **optional, codec-agnostic** task-concordance axis: a separate
...
LQS uses SemVer-style versioning on the spec, independent of the crate version.
...
A codec is **LQS-v1.0 conformant** if:
...
envelope**: `encode` returns `b"LQSX" + n_chan:u32 + n_samples:u32 + dtype:u8 +
...
(`lamquant_codec/lqs.py`) lets a Python pipeline grade against LQS without a Rust

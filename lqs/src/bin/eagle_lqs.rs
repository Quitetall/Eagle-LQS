//! `eagle-lqs` — CLI for the LQS universal EEG-compression benchmark
//! (LQS v1.0; see `SPEC/LQS-v1.0.md`).
//!
//! Subcommands (run `eagle-lqs <cmd> --help` for each):
//!
...
//!
//! Legacy form (preserved): `eagle-lqs <store|gzip|quantize> [FILE.edf]` grades
...
use lqs::corpus::{self, sha256_hex, CorpusManifest};
use lqs::report::{CodecIdentity, CorpusIdentity, LqsReport, LqsSubmission};
use lqs::subprocess::write_edf_bytes;
use lqs::{charts, edf, harness, manifest, report_html, stats, term};
...
#[command(
    name = "eagle-lqs",
    version,
    about = "LQS — the universal, vendor-neutral EEG-compression benchmark",
    long_about = "Grade any EEG codec (any language, via the file-based CLI contract) over a \
                  hash-pinned corpus and get a single comparable verdict. See SPEC/LQS-v1.0.md.\n\n\
                  Legacy form (preserved): eagle-lqs <store|gzip|quantize> [FILE.edf]"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
fn main() -> ExitCode {
    ...
    if !is_clap {
        // Legacy `eagle-lqs <codec> [file.edf]` (or no args -> synthetic).
...
fn resolve_target(
    ...
    if let Some(path) = codec_manifest {
        ...
        let codec = match m.into_adapter() {
            ...
            None => {
                eprintln!(
                    "error: codec '{}' command could not be resolved (set $LQS_CODEC_<NAME>_BIN or fix `cmd`)",
                    m.codec.name
                );
                return Err(ExitCode::from(5));
            }
...
fn grade_corpus_with_progress(
    ...
    label: &str,
    show: bool,
) -> Result<(Vec<LqsReport>, harness::CorpusSummary), corpus::CorpusError> {
    let pb = if show {
...
fn cmd_grade(a: GradeArgs) -> ExitCode {
    ...
    if let Some(corpus_path) = &a.corpus_manifest {
        ...
        let ranked: Vec<&LqsReport> = reports.iter().collect();
...
fn cmd_bench(a: BenchArgs) -> ExitCode {
    ...
    println!(
        "LQS bench — {} codecs over {} v{} ({} files, repeat={})\n",
        codecs.len(),
        manifest.name,
        manifest.version,
        manifest.file.len(),
    ...
    let mut subs: Vec<LqsSubmission> = Vec::new();
...
fn render_bench_table(subs: &[LqsSubmission], target: &str, color: bool) -> String {
    // Rank order.
...
fn cmd_legacy(args: &[String]) -> ExitCode {
    ...
    let codec: Box<dyn Codec> = match codec_name.as_str() {
        ...
        other => {
            ...
            eprintln!("(for the full CLI run `eagle-lqs --help`)");
...
fn write_outputs(subs: &[LqsSubmission], out: Option<&Path>, report: Option<&Path>) {

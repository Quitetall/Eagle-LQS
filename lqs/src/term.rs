...
/// A one-token grade badge, e.g. `LQS-L` / `— below floor`, colored by tier.
...
/// A compact, fixed-width-friendly grade label (`LQS-L` / `—`) colored by
...
pub fn render_report(rep: &LqsReport, color: bool) -> String {
...
pub fn render_leaderboard(reports: &[&LqsReport], color: bool) -> String {
    let dim = |s: &str| paint(s, color, |x| x.dim());
    let mut s = String::new();
    s.push_str(&paint("LQS leaderboard (best first)\n", color, |x| x.bold()));
    ...
    for (i, r) in reports.iter().enumerate() {
        let g = if r.grade == '\0' {
            ...
            grade_paint(r.grade, color, &format!("LQS-{}", r.grade))
        };
        s.push_str(&format!(
...
mod tests {
    ...
    fn plain_report_has_no_ansi() {
        ...
        assert!(out.contains("LQS-L"));
        assert!(out.contains("store"));
        assert!(out.contains("lqs-smoke"));
        // Boxed frame present.

...
//! adapters.
//!
//! Both the real-codec adapter ([`crate::adapters_lamquant`], which shells
//! out to the `lml` binary) and the generic external-codec adapter
//! ([`crate::adapters_external`], which drives any conformant codec via the
//! file-based CLI contract of `SPEC/LQS-v1.0.md` §6) need the same handful
//! of building blocks:
//!
...
impl SampleDtype {
    ...
    /// passed to a codec via the `--dtype` flag and `LQS_DTYPE` env var.

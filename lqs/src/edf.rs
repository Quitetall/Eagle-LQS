...
//! `.edf` file so the LQS benchmark CLI can grade real recordings instead
//! of only the built-in synthetic fixture. The LQS codec operates on the
...
//! ## Uniform-rate policy
//!
//! EDF permits a different rate per signal. LQS grades a single `fs`, so
                    ...
                    return Err(Error::new(
                        ...
                        format!(
                            "EDF: non-uniform sampling rates ({r} Hz vs {fs} Hz on \
                             {label:?}); LQS requires one shared fs",
                            label = labels[i]
                        ),
                    ));
                }

...
//!
//! [`render`] turns one or more [`LqsSubmission`]s (one per codec) into a
...
use crate::charts;
use crate::report::{LqsReport, LqsSubmission};
...
pub fn render(submissions: &[LqsSubmission]) -> String {
    ...
    h.push_str(&format!("<title>LQS report — {}</title>\n", esc(&corpus)));
    h.push_str(STYLE);
    h.push_str("</head>\n<body>\n<main>\n");
    h.push_str(&format!(
        "<h1>LQS report <span class=\"sub\">v{}</span></h1>\n<p class=\"meta\">corpus: <b>{}</b> · {} codec(s)</p>\n",
        esc(&spec),
        esc(&corpus),
        submissions.len()
    ));
    ...
    h.push_str("<footer>Generated by <code>eagle-lqs</code> · LQS v");
    h.push_str(&esc(&spec));
...
mod tests {
    ...
    fn submission(name: &str) -> LqsSubmission {
        ...
        LqsSubmission::new(
            ...
            CorpusIdentity { name: "lqs-smoke".into(), version: "1.0.0".into() },
            reports,
            summary,
        )
    }

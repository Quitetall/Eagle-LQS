...
//!
//! Where [`crate::adapters_lamquant`] hard-wires the `lml` binary, this
//! module drives an *arbitrary* codec through the file-based CLI contract
//! of `SPEC/LQS-v1.0.md` §6: a codec is any executable exposing
//!
//! ```text
...
//!
//! ```text
//! magic     : 4 bytes = b"LQSX"
//! n_chan    : u32 LE
//! n_samples : u32 LE   (per-channel sample count; 0 only valid for lqs0 output)
...
//! (matching [`crate::adapters_lamquant`]).
//!
//! [`encode`]: Codec::encode
//! [`decode`]: Codec::decode
...
const ENVELOPE_MAGIC: &[u8; 4] = b"LQSX";
...
pub enum InputFormat {
    ...
    #[default]
    Edf,
    /// The LQS0 reference container (ragged / empty channels survive).
    Lqs0,
}
...
pub enum OutputFormat {
    ...
    #[default]
    Raw,
    /// The LQS0 reference container (carries its own shape).
    Lqs0,
}
...
pub struct ExternalCodec {
    ...
    /// Extra environment merged over the `LQS_*` variables.
...
impl ExternalCodec {
    ...
    /// Set extra environment variables (merged over the `LQS_*` set).
...
impl ExternalCodec {
    ...
    fn run(
        ...
        cmd.args(&self.prefix_args)
            ...
            .env("LQS_CHANNELS", n_chan.to_string())
            .env("LQS_SAMPLES", n_samples.to_string())
            .env("LQS_RATE", format!("{fs}"))
            .env("LQS_DTYPE", self.sample_dtype.as_token())
            .env("LQS_WORKDIR", workdir);
        for (k, v) in &self.env {
            cmd.env(k, v);
        }
    ...
    fn try_encode(&self, signal: &[Vec<i64>], fs: f64) -> Vec<u8> {
        ...
        let input_bytes = match self.input_format {
            ...
            InputFormat::Lqs0 => serialize(signal),
        };
    ...
    fn try_decode(&self, blob: &[u8]) -> Vec<Vec<i64>> {
        ...
        match self.output_format {
            ...
            OutputFormat::Lqs0 => deserialize(&out),
        }
    }
}
...
///
/// Order: `$LQS_CODEC_<NAME>_BIN` (uppercased, `-`→`_`) if it points at a
...
pub fn resolve_cmd(name: &str, cmd: &str) -> Option<PathBuf> {
    let envvar = format!("LQS_CODEC_{}_BIN", name.to_uppercase().replace('-', "_"));

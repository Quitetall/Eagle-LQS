...
`lqs-smoke.toml` + `smoke/*.edf` is a tiny, **synthetic, redistributable** EEG
corpus so LQS runs offline with zero external data. It is **not** patient data —
...
```bash
cargo run --bin eagle-lqs -- verify-corpus --corpus-manifest corpora/lqs-smoke.toml
cargo run --bin eagle-lqs -- grade --codec gzip --corpus-manifest corpora/lqs-smoke.toml --out /tmp/sub.json
```

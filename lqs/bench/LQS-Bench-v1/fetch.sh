#!/bin/sh
# Fetch the LQS-Bench-v1 corpus (PhysioNet CHB-MIT v1.0.0, ODC-BY 1.0).
#
# The corpus data is NOT redistributed in this repo — only the record list and
# the pinned hashes (LQS-Bench-v1.toml) are. This script downloads the exact
...
# committed LQS-Bench-v1.toml paths resolve no matter the caller's cwd.
...
Verify against the committed pins (SHA-256 + shape):
  eagle-lqs verify-corpus --corpus-manifest "$HERE/LQS-Bench-v1.toml"

Benchmark your codec:
  eagle-lqs bench --codec-manifest YOUR_CODEC.toml \\
      --corpus-manifest "$HERE/LQS-Bench-v1.toml" --report report.html --charts
EOF

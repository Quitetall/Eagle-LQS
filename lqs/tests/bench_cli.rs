//! Integration test driving the real `eagle-lqs` binary end to end.
...
fn bin() -> &'static str {
    env!("CARGO_BIN_EXE_eagle-lqs")
}

fn smoke_manifest() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("corpora/lqs-smoke.toml")
}
...
fn legacy_form_still_works() {
    // `eagle-lqs gzip` (no subcommand) grades the synthetic fixture -> L -> exit 0.
    let out = Command::new(bin()).arg("gzip").output().expect("run legacy");
    assert!(out.status.success(), "legacy gzip should exit 0");
    let text = String::from_utf8_lossy(&out.stdout);
    assert!(text.contains("LQS-L"), "legacy output shows the grade");
}

//! Golden test: the `LqsSubmission` JSON wire format is byte-stable.
//!
...
//!
//! Regenerate intentionally with `LQS_REGEN_GOLDEN=1 cargo test --test
...
use lqs::harness;
use lqs::report::{CodecIdentity, CorpusIdentity, LqsSubmission};
...
fn build_submission() -> LqsSubmission {
...
fn submission_matches_golden() {
    ...
    if std::env::var("LQS_REGEN_GOLDEN").is_ok() || !path.exists() {
    ...
    assert_eq!(
        json, golden,
        "LqsSubmission JSON drifted from the golden. If intentional, regenerate with \
         LQS_REGEN_GOLDEN=1 cargo test --test submission_golden and review the diff."
    );
}
...
fn submission_round_trips_through_json() {
    ...
    let back = LqsSubmission::from_json(&sub.to_json()).expect("round-trips");
    assert_eq!(back, sub);
    assert_eq!(back.spec_version, lqs::SPEC_VERSION);

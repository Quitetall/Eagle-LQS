...
//! LQS-L. This is the load-bearing proof that "any codec, any language" can
...
use lqs::adapters_external::{ExternalCodec, InputFormat, OutputFormat};
use lqs::harness;
use lqs::manifest::load_codec_manifest;
use lqs::subprocess::{SampleDtype, ScratchDir};
...
/// A gzip codec over the LQS0 container: encode gzips the input, decode
...
const GZIP_LQS0_SH: &str = r#"case "$1" in
  encode) gzip -c "$2" > "$3" ;;
  decode) gunzip -c "$2" > "$3" ;;
  *) exit 2 ;;
esac
...
fn gzip_subprocess_via_builder_grades_l() {
    ...
    let script = write_script(&dir.path, "gzip_codec.sh", GZIP_LQS0_SH);
    ...
    let codec = ExternalCodec::new("gzip-ext", "sh")
        ...
        .with_formats(InputFormat::Lqs0, OutputFormat::Lqs0);
...
/// subprocess: encode copies the LQS0 input to the blob; decode parses LQS0
...
fn raw_output_python_codec_grades_l() {
    ...
    let codec = ExternalCodec::new("store-raw-py", "python3")
        ...
        .with_formats(InputFormat::Lqs0, OutputFormat::Raw);
